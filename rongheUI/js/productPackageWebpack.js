...
class ProductPackageAdd extends React.Component {
    ...
    rule (_data) {
        ...
        const _feeModesList = [];
        for (let i = 0;i < window.productPackagePageData.feeModes.listRule.length;i++) {
            if (newData.eaName.v === window.productPackagePageData.feeModes.listRule[i].eaName) {
                newData.feeModes.a.map((item) => {
                    if (window.productPackagePageData.feeModes.listRule[i].result.find((n) => (n === item.v))) {
        ...
        for (let i = 0;i < window.productPackagePageData.amount.showRule.length;i++) {
            if (newData.feeTypes.v === window.productPackagePageData.amount.showRule[i].feeTypes) {
                newShow.amount = window.productPackagePageData.amount.showRule[i].result;
                break;
            }
        }
        for (let i = 0;i < window.productPackagePageData.price.showRule.length;i++) {
            if (newData.feeTypes.v === window.productPackagePageData.price.showRule[i].feeTypes) {
                newShow.price = window.productPackagePageData.price.showRule[i].result;
                break;
            }
        }
        for (let i = 0;i < window.productPackagePageData.ladderPrice.showRule.length;i++) {
            if (newData.feeTypes.v === window.productPackagePageData.ladderPrice.showRule[i].feeTypes) {
                newShow.ladderPrice = window.productPackagePageData.ladderPrice.showRule[i].result;
                break;
            }
        }
        const newUnit = {
        ...
        for (let i = 0;i < window.productPackagePageData.amount.unitRule.length;i++) {
            if (newData.feeTypes.v === window.productPackagePageData.amount.unitRule[i].feeTypes && newData.feeModes.v === window.productPackagePageData.amount.unitRule[i].feeModes) {
                newUnit.amount = window.productPackagePageData.amount.unitRule[i].result;
                break;
            }
        }
        for (let i = 0;i < window.productPackagePageData.price.unitRule.length;i++) {
            if (newData.feeTypes.v === window.productPackagePageData.price.unitRule[i].feeTypes && newData.feeModes.v === window.productPackagePageData.price.unitRule[i].feeModes) {
                newUnit.price = window.productPackagePageData.price.unitRule[i].result;
                break;
            }
        }
        for (let i = 0;i < window.productPackagePageData.ladderPrice.unitRule.length;i++) {
            if (newData.feeTypes.v === window.productPackagePageData.ladderPrice.unitRule[i].feeTypes && newData.feeModes.v === window.productPackagePageData.ladderPrice.unitRule[i].feeModes) {
                newUnit.ladderPrice = window.productPackagePageData.ladderPrice.unitRule[i].result;
                break;
            }
        }
        ...
        return (
            <div className='productPackageArea'>
                ...
                {this.state.show.amount ? (
                    <div className='item fl'>
                        <span>规格：</span>
                        <UIInput type='positiveInt' className='inputBtn' maxValue={window.productPackagePageData.amount.maxValue} maxLength={window.productPackagePageData.amount.maxLength} placeholder={window.productPackagePageData.amount.placeholder} v={this.state.data.amount} onBlur={this.returnAmountValue}/>
